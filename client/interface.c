...
void init_interfaces() {
  ...
  for (interface = interfaces; interface && interface->if_index; interface++) {
    ...
    if (!valid_addr || (addr[0] == 0 && addr[1] == 0 && addr[2] == 0 && addr[3] == 0 && addr[4] == 0 && addr[5] == 0)) {
            valid_addr = 0;
        addr[0] = (unsigned char)( random() % 0xFF);
        addr[1] = (unsigned char)( random() % 0xFF);
        addr[2] = (unsigned char)( random() % 0xFF);
        addr[3] = (unsigned char)( random() % 0xFF);
        addr[4] = (unsigned char)( random() % 0xFF);
        addr[5] = (unsigned char)( random() % 0xFF);
    }
